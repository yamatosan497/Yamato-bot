...
async def music_handler(client: Client, message: Message):
    \"\"\"Captura archivos de música y envía UNA sola confirmación global por usuario.
    Se agrupan todos los mensajes recibidos en un corto periodo y luego se pide confirmación.
    \"\"\"
    # Verificamos tipo de archivo
    if not (message.audio or (message.document and getattr(message.document, "mime_type", "") and "audio" in message.document.mime_type)):
        return

    user_id = message.from_user.id

    # Estructura por usuario: {"files": [message_id,...], "task": asyncio.Task or None, "chat_id": int}
    if user_id not in MUSIC_QUEUE:
        MUSIC_QUEUE[user_id] = {"files": [], "task": None, "chat_id": message.chat.id}

    # Añadir el mensaje a la lista de archivos
    MUSIC_QUEUE[user_id]["files"].append(message.id)
    MUSIC_QUEUE[user_id]["chat_id"] = message.chat.id

    logger.info(f"Mensaje {message.id} agregado a la cola del usuario {user_id}. Total: {len(MUSIC_QUEUE[user_id]['files'])}")

    # Si ya hay una tarea programada para enviar la confirmación, cancelarla (la reemplazamos)
    task = MUSIC_QUEUE[user_id].get("task")
    if task and not task.done():
        task.cancel()

    async def wait_and_confirm(u_id=user_id, orig_message=message):
        try:
            # Esperar un breve periodo para recolectar todos los grupos enviados por Telegram
            await asyncio.sleep(2.0)  # tiempo en segundos que define "fin" del envío
        except asyncio.CancelledError:
            return

        # Reconfirmar que aún hay archivos
        if u_id not in MUSIC_QUEUE or len(MUSIC_QUEUE[u_id]["files"]) == 0:
            return

        num_files = len(MUSIC_QUEUE[u_id]["files"])
        question = f"🎶 {num_files} canción(es). ¿Guardar en Músicas?"
        keyboard = [[
            InlineKeyboardButton("✅ Sí", callback_data=f"save_music_{u_id}"),
            InlineKeyboardButton("❌ No", callback_data=f"cancel_music_{u_id}")
        ]]

        try:
            await orig_message.reply_text(question, reply_markup=InlineKeyboardMarkup(keyboard))
            logger.info(f"Enviada confirmación única al usuario {u_id} por {num_files} archivos.")
        except Exception as e:
            logger.error(f"Error al enviar confirmación a {u_id}: {e}")

        # Nota: no borramos la cola aquí; la cola se procesa en callbacks (save/cancel)

    # Programar la tarea y guardarla
    MUSIC_QUEUE[user_id]["task"] = asyncio.create_task(wait_and_confirm())
...
async def callback_handler(client: Client, callback_query: CallbackQuery):
    ...
    if data.startswith("save_music_") and user_id in MUSIC_QUEUE:
        ...
        try:
            ...
            for msg_id in MUSIC_QUEUE[user_id]["files"]:
